...
@RoutePage()
class PickingTaskDetailsScreen extends ConsumerStatefulWidget {
  final PutawayTaskDetailEntity taskDetail;

  const PickingTaskDetailsScreen({super.key, required this.taskDetail});

  @override
  ConsumerState<PickingTaskDetailsScreen> createState() =>
      _PickingTaskDetailsScreenState();
}

class _PickingTaskDetailsScreenState
    extends ConsumerState<PickingTaskDetailsScreen> {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (confirm == true && mounted) {
        await ref
            ...
            .confirmPicking(task: task, trip: trip);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
      ...
      next.when(
        ...
        success: (message) {
          if (mounted) {
            ...
            // List reload is handled in [ConfirmPickingViewModel] after confirm succeeds.
            Future.delayed(const Duration(seconds: 1), () {
              if (mounted) {
                context.router.popUntilRouteWithName('PickingTasksListRoute');
              }
            });
          }
        },
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Picking Task Details'),
